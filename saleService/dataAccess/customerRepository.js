...
const insertWithIntegrationOutbox = async function (customer) {
  try {
    ...
    let customerResult = await customerModel.create([customer], {
      session: session,
    });
    ...
    if (customerResult) {
      ...
      expirationTime.setMinutes(
        expirationTime.getMinutes() + config.expiration.customer
      );
      ...
      var integrationOutboxModel = context.model(
        'IntegrationOutbox',
        IntegrationOutbox,
        'IntegrationOutbox'
      );
      await integrationOutboxModel.create([integrationOutbox], {
        session: session,
      });
    }
...
const deleteCustomerAndLogTheEvent = async function (expiredItem) {
  try {
    ...
    await customerModel.deleteOne(
      { _id: expiredItem.targetObjectId },
      { session: session }
    );

    const integrationOutboxModel = context.model(
      'IntegrationOutbox',
      IntegrationOutbox,
      'IntegrationOutbox'
    );
    await integrationOutboxModel.updateOne(
      { _id: expiredItem._id },
      { isDone: true },
      { session: session }
    );
