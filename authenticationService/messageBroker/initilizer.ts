...
export default class Initilizer {
    ...
    public initilize(): void {
        amqpcb.connect(config.messageBroker.url, (error: any, connection: amqpcb.Connection) => connection.createChannel(this.createChannelCallback));
    }

    private createChannelCallback(error: any, channel: amqpcb.Channel): void {
        try {

            let customerQueue: Queue = new CustomerQueue(channel);
